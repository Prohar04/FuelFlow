 ...
 * @param {Date} periodStart - Start date of period
 * @param {Date} periodEnd - End date of period
 * @param {Number} hourlyRate - Hourly rate for the employee
...
export const calculateEmployeePayroll = async (
  ...
  // Calculate pay: Hourly Rate × Total Hours Worked - Deductions
...
export const generateMonthlyPayroll = async (
  ...
  for (const employee of employees) {
    ...
    // Use employee's hourly rate
    const hourlyRate = employee.hourlyRate || 0;

    if (hourlyRate === 0) {
      // Skip employees without hourly rate set
      continue;
    }
...
export const getPayrollSummary = async (pumpId, periodStart, periodEnd) => {
  ...
  const summary = {
    ...
    pendingPayrolls: payrolls.filter((p) => p.paymentStatus === "pending")
      .length,
