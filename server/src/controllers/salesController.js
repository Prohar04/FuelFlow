...
export const createSale = async (req, res, next) => {
  try {
    ...
    if (!fuelType || !quantity || !unitPrice) {
      return res
        ...
        .json(
          formatError(
            "VALIDATION_ERROR",
            "FuelType, quantity, and unitPrice are required"
          )
        );
    }
    ...
    // Generate receipt with retry mechanism for duplicate handling
    ...
    while (!receipt && attempts < maxAttempts) {
      ...
      try {
        ...
        // If duplicate key error (code 11000), retry with next number
    ...
    if (!receipt) {
      // Rollback sale if receipt creation failed
      await Sale.findByIdAndDelete(sale._id);
      return res
        .status(500)
        .json(
          formatError("RECEIPT_ERROR", "Failed to generate receipt number")
        );
    }
    ...
    return res
      .status(201)
      .json(formatSuccess(saleWithReceipt, "Sale created successfully"));
  } catch (error) {
    next(error);
  }
};
...
export const getSales = async (req, res, next) => {
  try {
    ...
    // Determine limit based on role - admin gets more data
...
export const getSaleById = async (req, res, next) => {
  try {
    ...
    if (!sale) {
      return res.status(404).json(formatError("NOT_FOUND", "Sale not found"));
    }
