...
export const createUser = async (req, res, next) => {
  try {
    ...
    if (role !== "admin" && !pumpId) {
      return res
        ...
        .json(
          formatError(
            "VALIDATION_ERROR",
            "Pump assignment is required for non-admin roles"
          )
        );
    }
      ...
      if (role === "admin" || role === "manager") {
        return res
          ...
          .json(
            formatError(
              "FORBIDDEN",
              "Managers can only create employee and cashier accounts (not managers or admins)"
            )
          );
      }
      ...
      if (pumpId !== req.user.pumpId?.toString()) {
        return res
          ...
          .json(
            formatError(
              "FORBIDDEN",
              "Managers can only create employees for their assigned pump"
            )
          );
      }
    }
    ...
    if (existingUser) {
      return res
        .status(409)
        .json(formatError("DUPLICATE_ERROR", "Email already exists"));
    }
    ...
    if (pumpId) {
      ...
      if (!pump) {
        return res.status(404).json(formatError("NOT_FOUND", "Pump not found"));
      }
    }
...
export const getUserById = async (req, res, next) => {
  try {
    ...
    if (!user) {
      return res.status(404).json(formatError("NOT_FOUND", "User not found"));
    }
...
export const updateUser = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { name, role, jobTitle, pumpId, hourlyRate } = req.body;
      ...
      if (role && role !== user.role) {
        return res
          ...
          .json(
            formatError(
              "FORBIDDEN",
              "You cannot change your own role. Contact a system administrator."
            )
          );
      }
      ...
      if (pumpId && pumpId !== user.pumpId?.toString()) {
        return res
          ...
          .json(
            formatError(
              "FORBIDDEN",
              "You cannot change your own pump assignment. Contact a system administrator."
            )
          );
      }
    }
    ...
    return res
      .status(200)
      .json(formatSuccess(updatedUser, "User updated successfully"));
  } catch (error) {
    next(error);
  }
};
  ...
  try {
    ...
    if (req.user.role === "manager") {
      if (user.pumpId?.toString() !== req.user.pumpId?.toString()) {
        return res
          ...
          .json(
            formatError(
              "FORBIDDEN",
              "Managers can only terminate employees from their pump"
            )
          );
      }
    } else if (req.user.role !== "admin") {
      return res
        ...
        .json(
          formatError(
            "FORBIDDEN",
            "Only admin and managers can terminate users"
          )
        );
    }
    ...
    return res
      ...
      .json(
        formatSuccess(
          user,
          "User terminated successfully and notification email sent"
        )
      );
  } catch (error) {
    next(error);
...
export const reinstateUser = async (req, res, next) => {
  try {
    ...
    if (user.status !== "terminated") {
      return res
        .status(400)
        .json(formatError("INVALID_OPERATION", "User is not terminated"));
    }
    ...
    if (req.user.role === "manager") {
      if (user.pumpId?.toString() !== req.user.pumpId?.toString()) {
        return res
          ...
          .json(
            formatError(
              "FORBIDDEN",
              "Managers can only reinstate employees from their pump"
            )
          );
      }
    } else if (req.user.role !== "admin") {
      return res
        ...
        .json(
          formatError(
            "FORBIDDEN",
            "Only admin and managers can reinstate users"
          )
        );
    }
    ...
    return res
      ...
      .json(
        formatSuccess(
          user,
          "User reinstated successfully and notification email sent"
        )
      );
  } catch (error) {
    next(error);
