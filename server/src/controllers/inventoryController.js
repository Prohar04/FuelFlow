...
export const getInventory = async (req, res, next) => {
  try {
    ...
    // For admin without pumpId, get inventory for all pumps
    if (req.user.role === "admin" && !pumpId) {
      // Build aggregation pipeline for all pumps
      const matchStage = {};
      ...
      return res.status(200).json(formatSuccess(stockLevels));
    }
    ...
    // Convert to ObjectId for proper aggregation matching
    const pumpObjectId = new mongoose.Types.ObjectId(queryPumpId);

    // Build aggregation pipeline to calculate current stock
    const matchStage = { pumpId: pumpObjectId };
    if (fuelType) matchStage.fuelType = fuelType;
    ...
    const configs = await InventoryConfig.find({ pumpId: pumpObjectId });
    const configMap = new Map(
      configs.map((c) => [c.fuelType, c.lowStockThreshold])
    );
...
export const createStockIn = async (req, res, next) => {
  try {
    ...
    return res
      .status(201)
      .json(formatSuccess(entry, "Stock-in entry created successfully"));
  } catch (error) {
    next(error);
  }
};
...
export const createAdjustment = async (req, res, next) => {
  try {
    ...
    return res
      .status(201)
      .json(formatSuccess(entry, "Inventory adjustment created successfully"));
  } catch (error) {
    next(error);
  }
};
...
export const setLowStockThreshold = async (req, res, next) => {
  try {
    ...
    if (!fuelType || lowStockThreshold === undefined) {
      return res
        ...
        .json(
          formatError(
            "VALIDATION_ERROR",
            "FuelType and lowStockThreshold are required"
          )
        );
    }
    ...
    return res
      .status(200)
      .json(formatSuccess(config, "Low stock threshold updated successfully"));
  } catch (error) {
    next(error);
  }
};
