...
export default function EmployeeDetailsModal({
  ...
  const handleUpdateSalary = async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error?.message || "Failed to update hourly rate"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleUpdatePump = async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error?.message || "Failed to update pump"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleTerminate = async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error?.message || "Failed to terminate employee"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleReinstate = async () => {
    if (
      !confirm(
        "Are you sure you want to reinstate this employee? They will receive an email notification."
      )
    ) {
      return;
    }
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error?.message || "Failed to reinstate employee"
      );
    } finally {
      setLoading(false);
    }
  ...
  const tabs = [
    ...
    { id: "salary", label: "Hourly Rate", icon: DollarSign },
            ...
            <div className='tab-panel'>
              <div className='info-grid'>
                ...
                <div className='info-item'>
                  ...
                  <p>
                    {employee.pumpId?.name || "N/A"}{" "}
                    {employee.pumpId?.code ? `(${employee.pumpId.code})` : ""}
                  </p>
                </div>
              </div>
              ...
              {employee.status === "active" && isSelf && (
                ...
                >
                  ⚠️ You cannot terminate your own account. Contact a system
                  administrator.
                </div>
              )}
            ...
            <div className='tab-panel'>
              <div className='form-section'>
                ...
                <p className='section-description'>
                  Set the hourly rate for this employee. This will be used for
                ...
                <div className='current-salary'>
                  ...
                  <strong>
                    ৳{employee.hourlyRate?.toLocaleString() || "0"}/hour
                  </strong>
                </div>
                <Button
                  ...
                  loading={loading}
                  fullWidth
                  disabled={formData.hourlyRate === employee.hourlyRate}
                >
                  Update Hourly Rate
                </Button>
              </div>
          ...
          {activeTab === "pump" && (
            <div className='tab-panel'>
              <div className='form-section'>
                ...
                <p className='section-description'>
                  Change the pump this employee is assigned to. This will update
                  their work location.
                </p>
                <div className='input-wrapper'>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, pumpId: e.target.value })
                    }
                ...
                <Button
                  ...
                  disabled={
                    formData.pumpId ===
                      (employee.pumpId?._id || employee.pumpId) ||
                    cannotEditCriticalFields
                  }
                >
                  ...
                  {employee.employmentHistory.map((record, index) => (
                    <div key={index} className='history-item'>
                      ...
                      <div className='history-details'>
                        ...
                        <p>
                          <strong>Job Title:</strong> {record.jobTitle || "N/A"}
                        </p>
                        {record.reason && (
                          <p>
