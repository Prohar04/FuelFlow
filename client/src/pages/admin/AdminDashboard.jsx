...
export default function AdminDashboard() {
  ...
  const totalRevenue = sales.reduce(
    (sum, sale) => sum + (sale.total || sale.quantity * sale.unitPrice || 0),
    0
  );
  ...
  const pendingOrders = orders.filter(
    (o) =>
      o.status === "created" || o.status === "emailed" || o.status === "pending"
  ).length;
  ...
  // Total fuel sold
  const totalFuelSold = sales.reduce(
  ...
  const salesByDate = sales.reduce((acc, sale) => {
    ...
    if (!acc[date]) {
      acc[date] = { date, revenue: 0, count: 0, quantity: 0 };
    }
    acc[date].revenue += sale.total || sale.quantity * sale.unitPrice || 0;
    acc[date].count += 1;
  ...
  const salesByFuelType = sales.reduce((acc, sale) => {
    ...
    if (!acc[type]) {
      acc[type] = {
        ...
        value: 0,
        quantity: 0,
        fill: FUEL_COLORS[type] || COLORS[0],
      };
    }
    acc[type].value += sale.total || sale.quantity * sale.unitPrice || 0;
  ...
  const salesByPump = sales.reduce((acc, sale) => {
    ...
    if (!acc[pumpName]) {
      acc[pumpName] = { name: pumpName, sales: 0, revenue: 0, quantity: 0 };
    }
    acc[pumpName].sales += 1;
    acc[pumpName].revenue += sale.total || sale.quantity * sale.unitPrice || 0;
    acc[pumpName].quantity += sale.quantity || 0;
    return acc;
  }, {});
  ...
  const cashierSales = sales.reduce((acc, sale) => {
    ...
    if (cashierId) {
      ...
      acc[cashierId].revenue +=
        sale.total || sale.quantity * sale.unitPrice || 0;
      acc[cashierId].quantity += sale.quantity || 0;
    }
    return acc;
  }, {});
    ...
    <DashboardLayout role='admin'>
      <div style={{ display: "grid", gap: "var(--spacing-lg)" }}>
        ...
        {/* <Card
          title='Revenue Trend'
          icon={<Activity />}
          subtitle={`Last ${Math.min(revenueTrend.length, 14)} days`}
        >
